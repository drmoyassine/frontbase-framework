...
| CF-15 | **RULE 8 — mutation harness** (every security gate proven RED-on-break) | Phase 2 audit | M3.0 | ✅ DONE 2026-07-10; **extended 2026-07-11**: 12 → **19 proofs** after adding the auth/DB gates (password-verify, session-forgery, seed-idempotency, hash-no-leak, canActOnTenant, setup post-init lock) + the CF-19 deploy-seed **no-argv-leak** gate. `pnpm -r test:mutation` |
