...
| CF-18 | **Admin console React UI** — shell + login + dashboard + pages + tenants | M-ID.3 / M3.DB | M-ID.3-UI | 🟡 **Phase 1 (MVP) DONE 2026-07-12** — new package `@frontbase/admin-console` (React+Vite+Tailwind+shadcn): image-4-style sidebar shell (ported from the product), Login, Dashboard, Pages (draft JSON editor + preview + Save/Publish), Tenants (list/create). Inlined into the worker at `/console` via `virtual:spa-bundle` (single-artifact, 316 KB gzip total); no-leak + mutation gated (RULE 1/8). **Phase 2+**: visual canvas editor (consume `@frontbase/builder` — deferred; `BuilderWorkspace` is a component-prop editor, not page-layout, and the full visual canvas is its own future milestone), + the backend-light features (Data Studio, Users, Storage, Automations, Edge, Settings) which need framework backend growth per the CF-21 audit. cf-full's old `admin.ts` shim removed |
