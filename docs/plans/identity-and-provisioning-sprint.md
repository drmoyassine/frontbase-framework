...
**ID.1.6 — Wire into `createConsole` (`index.ts`).** Mount `login`/`logout` BEFORE `app.use('*', defaultDenyAuth)`; mount `me` AFTER. `createConsole` gains a `sessionSecret` dep; it builds `resolvePrincipal` from `createResolvePrincipal({jwtSecret: deps.sessionSecret, jwtCookie:'fb_session'})` if the caller didn't pass one. Call `migrateUp` + `seedOwner` at construction (behind a `deps.seed?: {email,password}` read from env by the worker). Gate: the existing `console.mjs`/`authz.mjs` stay green; a new `backend/test/login-e2e.mjs` proves: unauth `/pages` → 401; `POST /login` (seeded creds) → cookie; same cookie on `/pages` → 200.
