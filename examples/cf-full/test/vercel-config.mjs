...
console.log('=== vercel function artifact: tracked byte-sync ===');
// Vercel collects api/ functions from the UPLOADED source, before the build
// command runs — a generated-only function never deploys via git integration.
const apiCmsPath = join(exampleRoot, 'api', 'cms.mjs');
check('api/cms.mjs is tracked (in the upload snapshot Vercel builds functions from)',
    spawnSync('git', ['ls-files', '--error-unmatch', 'api/cms.mjs'], { cwd: exampleRoot }).status === 0
    && existsSync(apiCmsPath));
if (existsSync(join(exampleRoot, 'dist', 'vercel.mjs'))) {
    check('api/cms.mjs is byte-identical to dist/vercel.mjs (regenerated by the build)',
        readFileSync(apiCmsPath).equals(readFileSync(join(exampleRoot, 'dist', 'vercel.mjs'))));
} else {
    console.log('  ⏭️  byte-sync skipped (dist/vercel.mjs absent — run the build first)');
}
