...
No pre-build needed: the image runs `pnpm -r build`, which stages the console
(`packages/console` source → `console-dist/frontbase-admin`) and the hydration
bundle (`console-dist/react`) itself.
