...
export function registerAuthFormsRoutes(app: App, runner: DbRunner, now: () => string): void {
    ...
    app.get('/api/auth-forms/primary/', async (c) => {
        const r = await runner.query(`SELECT ${COLS} FROM auth_forms WHERE tenant_slug = ? AND is_primary = 1`, [c.get('tenant')]);
        return r[0] ? c.json({ success: true, data: r[0], error: null }) : c.json({ detail: 'No primary form' }, 404);
    });
    app.get('/api/auth-forms/:form_id/', async (c) => {
        const r = await runner.query(`SELECT ${COLS} FROM auth_forms WHERE tenant_slug = ? AND id = ?`, [c.get('tenant'), c.req.param('form_id')]);
        return r[0] ? c.json({ success: true, data: r[0], error: null }) : c.json({ detail: 'Not found' }, 404);
    });
