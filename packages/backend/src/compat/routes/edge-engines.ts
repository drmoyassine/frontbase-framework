...
export function registerEdgeEnginesRoutes(app: App, p2: (t: string) => Phase2Store, now: () => string): void {
    app.get('/api/edge-engines/', async (c) => c.json({
        engines: (await p2(c.get('tenant')).listEdgeResources('engine')).map((r) => serializeEngine(r)),
    }));
    ...
    const noopBatch = async (c: { req: { json: () => Promise<unknown> } }) =>
        batchOver(await c.req.json().catch(() => ({})), async () => {});
    ...
    app.post('/api/edge-engines/:engine_id/test', (c) => c.json(testResult(true, 'This deployment is the engine; it is reachable')));
    ...
    app.get('/api/edge-engines/:engine_id/rotation-history', (c) => c.json({ history: [] }));
    app.post('/api/edge-engines/:engine_id/rollback-rotation', (c) => c.json({ success: true }));
    app.get('/api/edge-engines/:engine_id/source', (c) => c.json({ success: true, files: [], file_count: 0, total_size: 0 }));
    app.put('/api/edge-engines/:engine_id/source', (c) => c.json({ success: true }));
    app.post('/api/edge-engines/:engine_id/export', (c) => c.json({}));
    app.post('/api/edge-engines/:engine_id/finalize-move', (c) => c.json({ finalized: true, engine_id: c.req.param('engine_id') }));
    app.post('/api/edge-engines/:engine_id/cancel-move', (c) => c.json({ cancelled: true, engine: null }));
    ...
    app.get('/api/edge-engines/:engine_id/agent-profiles', (c) => c.json({ profiles: [], total: 0 }));
