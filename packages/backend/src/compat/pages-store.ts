...
export interface CompatPageRow {
    ...
    keywords: string | null; is_public: number; is_homepage: number; layout_data: string;
...
export function serializePage(r: CompatPageRow): Record<string, unknown> {
    ...
    return {
        ...
        contentHash: r.content_hash, hasUnpublishedChanges: false, deployments: [],
    };
}
...
export class PagesStore {
    ...
    async list(): Promise<CompatPageRow[]> {
        return await this.runner.query(
            'SELECT id, name, slug, title, description, keywords, is_public, is_homepage, layout_data, seo_data, deleted_at, content_hash, created_at, updated_at FROM compat_pages WHERE tenant_slug = ? AND deleted_at IS NULL ORDER BY created_at',
            [this.tenant],
        ) as unknown as CompatPageRow[];
    }
    async get(id: string): Promise<CompatPageRow | null> {
        const rows = await this.runner.query(
            'SELECT id, name, slug, title, description, keywords, is_public, is_homepage, layout_data, seo_data, deleted_at, content_hash, created_at, updated_at FROM compat_pages WHERE tenant_slug = ? AND id = ?',
            [this.tenant, id],
        );
    ...
    async getBySlug(slug: string, includeDeleted = false): Promise<CompatPageRow | null> {
        const rows = await this.runner.query(
            `SELECT id, name, slug, title, description, keywords, is_public, is_homepage, layout_data, seo_data, deleted_at, content_hash, created_at, updated_at FROM compat_pages WHERE tenant_slug = ? AND slug = ? ${includeDeleted ? '' : 'AND deleted_at IS NULL'}`,
            [this.tenant, slug],
        );
        return (rows[0] as unknown as CompatPageRow) ?? null;
    }
    async homepage(): Promise<CompatPageRow | null> {
        const rows = await this.runner.query(
            'SELECT id, name, slug, title, description, keywords, is_public, is_homepage, layout_data, seo_data, deleted_at, content_hash, created_at, updated_at FROM compat_pages WHERE tenant_slug = ? AND is_homepage = 1 AND deleted_at IS NULL',
            [this.tenant],
        );
        return (rows[0] as unknown as CompatPageRow) ?? null;
    }
